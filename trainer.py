...
class Trainer(object):
  ...
  def train_step(
    ...
    for step, sample_batched in steps:
      ...
      steps.set_description(f'Epoch [{epoch+1:02}/{self.args.epoch}][train] steps: {step_number}')
      steps.set_postfix(loss=epoch_loss / (step + 1))

    return epoch_loss / len(train_dataloader), loss.item()
    ...
    with torch.no_grad():
      for step, sample_batched in steps:
        ...
        if epoch is not None:
          steps.set_description(f'[{epoch+1:02}/{self.args.epoch}][evaluate]')
          steps.set_postfix(loss=epoch_loss / (step + 1))
  ...
  def train(
    ...
    for epoch in range(self.args.epoch):
      ...
      train_loss, loss = self.train_step(model=model, epoch=epoch, train_dataloader=train_dataloader)
      ...
      print(f'Epoch: {epoch+1:02} | Time: {epoch_mins}m {epoch_secs}s | train_loss: {train_loss:02} valid_loss: {valid_loss:02} | valid_acc: {accuracy} | valid_f1: {f1}')
